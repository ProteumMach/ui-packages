/* tslint:disable */
/* eslint-disable */
/**
 * Toolpath Engine API
 * Public REST API for Toolpath engine operations.  **Units.** All dimensional values — in parts and in feature details — are in millimetres; all angles are in degrees. Each part response also states this in its `units` field.  **Browser use.** CORS is per API key: a request only receives an `Access-Control-Allow-Origin` header when its `Origin` is in the allowlist configured on the key used (set when creating or editing the key in the portal). A key with no allowed origins is server-to-server only. A Bearer API key placed in client-side JavaScript is readable by anyone — restrict its origins, prefer a `read_only` key (read_only keys may call GET operations only; writes are rejected with 403), or proxy requests through your own server.
 *
 * The version of the OpenAPI document: 1.3.1
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js'
/**
 * Access direction from which the kernel extracted this feature.
 * @export
 * @interface PartFeatureMachiningDirection
 */
export interface PartFeatureMachiningDirection {
  /**
   * X coordinate or direction component.
   * @type {number}
   * @memberof PartFeatureMachiningDirection
   */
  x: number
  /**
   * Y coordinate or direction component.
   * @type {number}
   * @memberof PartFeatureMachiningDirection
   */
  y: number
  /**
   * Z coordinate or direction component.
   * @type {number}
   * @memberof PartFeatureMachiningDirection
   */
  z: number
}

/**
 * Check if a given object implements the PartFeatureMachiningDirection interface.
 */
export function instanceOfPartFeatureMachiningDirection(
  value: object,
): value is PartFeatureMachiningDirection {
  if (!('x' in value) || value['x'] === undefined) return false
  if (!('y' in value) || value['y'] === undefined) return false
  if (!('z' in value) || value['z'] === undefined) return false
  return true
}

export function PartFeatureMachiningDirectionFromJSON(json: any): PartFeatureMachiningDirection {
  return PartFeatureMachiningDirectionFromJSONTyped(json, false)
}

export function PartFeatureMachiningDirectionFromJSONTyped(
  json: any,
  ignoreDiscriminator: boolean,
): PartFeatureMachiningDirection {
  if (json == null) {
    return json
  }
  return {
    x: json['x'],
    y: json['y'],
    z: json['z'],
  }
}

export function PartFeatureMachiningDirectionToJSON(json: any): PartFeatureMachiningDirection {
  return PartFeatureMachiningDirectionToJSONTyped(json, false)
}

export function PartFeatureMachiningDirectionToJSONTyped(
  value?: PartFeatureMachiningDirection | null,
  ignoreDiscriminator: boolean = false,
): any {
  if (value == null) {
    return value
  }

  return {
    x: value['x'],
    y: value['y'],
    z: value['z'],
  }
}
