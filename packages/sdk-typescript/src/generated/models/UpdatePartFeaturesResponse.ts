/* tslint:disable */
/* eslint-disable */
/**
 * Toolpath Engine API
 * Public REST API for Toolpath engine operations.  **Units.** All dimensional values — in parts and in feature details — are in millimetres; all angles are in degrees. Each part response also states this in its `units` field.  **Browser use.** CORS is per API key: a request only receives an `Access-Control-Allow-Origin` header when its `Origin` is in the allowlist configured on the key used (set when creating or editing the key in the portal). A key with no allowed origins is server-to-server only. A Bearer API key placed in client-side JavaScript is readable by anyone — restrict its origins, prefer a `read_only` key (read_only keys may call GET operations only; writes are rejected with 403), or proxy requests through your own server.
 *
 * The version of the OpenAPI document: 1.1.0
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js'
/**
 *
 * @export
 * @interface UpdatePartFeaturesResponse
 */
export interface UpdatePartFeaturesResponse {
  /**
   * Identifier of the queued enrichment job.
   * @type {string}
   * @memberof UpdatePartFeaturesResponse
   */
  jobId: string
  /**
   * Identifier of the part owning the requested features.
   * @type {string}
   * @memberof UpdatePartFeaturesResponse
   */
  partId: string
  /**
   * Initial state of the accepted enrichment job.
   * @type {UpdatePartFeaturesResponseStatusEnum}
   * @memberof UpdatePartFeaturesResponse
   */
  status: UpdatePartFeaturesResponseStatusEnum
}

/**
 * @export
 */
export const UpdatePartFeaturesResponseStatusEnum = {
  Queued: 'queued',
} as const
export type UpdatePartFeaturesResponseStatusEnum =
  (typeof UpdatePartFeaturesResponseStatusEnum)[keyof typeof UpdatePartFeaturesResponseStatusEnum]

/**
 * Check if a given object implements the UpdatePartFeaturesResponse interface.
 */
export function instanceOfUpdatePartFeaturesResponse(
  value: object,
): value is UpdatePartFeaturesResponse {
  if (!('jobId' in value) || value['jobId'] === undefined) return false
  if (!('partId' in value) || value['partId'] === undefined) return false
  if (!('status' in value) || value['status'] === undefined) return false
  if (value['status'] !== 'queued') return false

  return true
}

export function UpdatePartFeaturesResponseFromJSON(json: any): UpdatePartFeaturesResponse {
  return UpdatePartFeaturesResponseFromJSONTyped(json, false)
}

export function UpdatePartFeaturesResponseFromJSONTyped(
  json: any,
  ignoreDiscriminator: boolean,
): UpdatePartFeaturesResponse {
  if (json == null) {
    return json
  }
  return {
    jobId: json['jobId'],
    partId: json['partId'],
    status: json['status'],
  }
}

export function UpdatePartFeaturesResponseToJSON(json: any): UpdatePartFeaturesResponse {
  return UpdatePartFeaturesResponseToJSONTyped(json, false)
}

export function UpdatePartFeaturesResponseToJSONTyped(
  value?: UpdatePartFeaturesResponse | null,
  ignoreDiscriminator: boolean = false,
): any {
  if (value == null) {
    return value
  }

  return {
    jobId: value['jobId'],
    partId: value['partId'],
    status: value['status'],
  }
}
