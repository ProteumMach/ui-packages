/* tslint:disable */
/* eslint-disable */
/**
 * Toolpath Engine API
 * Public REST API for Toolpath engine operations.  **Units.** All dimensional values — in parts and in feature details — are in millimetres; all angles are in degrees. Each part response also states this in its `units` field.  **Browser use.** CORS is per API key: a request only receives an `Access-Control-Allow-Origin` header when its `Origin` is in the allowlist configured on the key used (set when creating or editing the key in the portal). A key with no allowed origins is server-to-server only. A Bearer API key placed in client-side JavaScript is readable by anyone — restrict its origins, prefer a `read_only` key (read_only keys may call GET operations only; writes are rejected with 403), or proxy requests through your own server.
 *
 * The version of the OpenAPI document: 1.3.3
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js'
import type { OffsetLength } from './OffsetLength.js'
import {
  OffsetLengthFromJSON,
  OffsetLengthFromJSONTyped,
  OffsetLengthToJSON,
  OffsetLengthToJSONTyped,
} from './OffsetLength.js'
import type { CdData } from './CdData.js'
import { CdDataFromJSON, CdDataFromJSONTyped, CdDataToJSON, CdDataToJSONTyped } from './CdData.js'

/**
 * A pocket: a walled space with a floor, and the blend where the two meet.
 * @export
 * @interface PocketFacts
 */
export interface PocketFacts {
  /**
   * Discriminator for this facts variant.
   * @type {PocketFactsKindEnum}
   * @memberof PocketFacts
   */
  kind: PocketFactsKindEnum
  /**
   * Clearance-diameter limits for tools sweeping the pocket.
   * @type {CdData}
   * @memberof PocketFacts
   */
  cd: CdData
  /**
   * The bottom diameter a terminal tool must not exceed; negative infinity when
   * unconstrained.
   * @type {number}
   * @memberof PocketFacts
   */
  maxBottomDiameter: number
  /**
   * The outline a pass around the wall follows, sized for any tool. Absent where the
   * projection left no outline — a wall nothing can price, not one that is free.
   * @type {OffsetLength}
   * @memberof PocketFacts
   */
  wallLength?: OffsetLength
  /**
   * Radius of the floor fillet; 0.0 when unfilleted.
   * @type {number}
   * @memberof PocketFacts
   */
  filletRadius: number
  /**
   * Height of the floor fillet; 0.0 when unfilleted.
   * @type {number}
   * @memberof PocketFacts
   */
  filletHeight: number
}

/**
 * @export
 */
export const PocketFactsKindEnum = {
  Pocket: 'Pocket',
} as const
export type PocketFactsKindEnum = (typeof PocketFactsKindEnum)[keyof typeof PocketFactsKindEnum]

/**
 * Check if a given object implements the PocketFacts interface.
 */
export function instanceOfPocketFacts(value: object): value is PocketFacts {
  if (!('kind' in value) || value['kind'] === undefined) return false
  if (value['kind'] !== 'Pocket') return false

  if (!('cd' in value) || value['cd'] === undefined) return false
  if (!('maxBottomDiameter' in value) || value['maxBottomDiameter'] === undefined) return false
  if (!('filletRadius' in value) || value['filletRadius'] === undefined) return false
  if (!('filletHeight' in value) || value['filletHeight'] === undefined) return false
  return true
}

export function PocketFactsFromJSON(json: any): PocketFacts {
  return PocketFactsFromJSONTyped(json, false)
}

export function PocketFactsFromJSONTyped(json: any, ignoreDiscriminator: boolean): PocketFacts {
  if (json == null) {
    return json
  }
  return {
    kind: json['kind'],
    cd: CdDataFromJSON(json['cd']),
    maxBottomDiameter: json['maxBottomDiameter'],
    wallLength: json['wallLength'] == null ? undefined : OffsetLengthFromJSON(json['wallLength']),
    filletRadius: json['filletRadius'],
    filletHeight: json['filletHeight'],
  }
}

export function PocketFactsToJSON(json: any): PocketFacts {
  return PocketFactsToJSONTyped(json, false)
}

export function PocketFactsToJSONTyped(
  value?: PocketFacts | null,
  ignoreDiscriminator: boolean = false,
): any {
  if (value == null) {
    return value
  }

  return {
    kind: value['kind'],
    cd: CdDataToJSON(value['cd']),
    maxBottomDiameter: value['maxBottomDiameter'],
    wallLength: OffsetLengthToJSON(value['wallLength']),
    filletRadius: value['filletRadius'],
    filletHeight: value['filletHeight'],
  }
}
