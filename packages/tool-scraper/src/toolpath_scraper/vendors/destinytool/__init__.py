...
Not a server-rendered vendor catalog: the storefront is a Next.js SPA built on
Firebase Studio with no product data in the HTML, so the "scrape" is a
paginated Firestore REST client rather than an HTML table parser. See
`scrape.py` for the endpoint and `records.py` for what the adapter derives
