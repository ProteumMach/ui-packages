...
A scraped CSV keeps **that vendor's own column labels**. Relabelling them into
a shared vocabulary on the way into the file would put a lie in the file whose
whole job is to record what the vendor published, and the labels really do not
line up: Kennametal's `D1_mm` and Destiny Tool's `cutDia_in` are the same
measurement under names neither vendor would recognise in the other's table.
Nothing reads a vendor's CSV but that vendor's own adapter.

So this is not a schema. It is the short list of conventions that hold *across*
the CSVs anyway, and the reason to make them explicit is that vendor #3 already
drifted from one:
...
Identity and units are the two worth enforcing; the rest are advisory, and are
here so that "advisory" is a decision on the page rather than an omission.

**A vendor's column label can collide with a real ISO 13399 code and mean
something else.** The standard defines `D1` as fixing hole diameter and `L` as
cutting edge length; Kennametal's tables use `D1` for the cutting diameter and
`L` for the overall length. The two vocabularies overlap on `D1`, `L`, `B`,
`H`, `RE`, `SIG` and `TP` among others, so anything that reads a vendor CSV
without going through that vendor's adapter can be confidently wrong rather
than obviously broken. See `records.GEOMETRY_FIELDS` for the canonical side of
that line.
"""
...
UnitSystem = Literal['millimeters', 'inches']
...
UNIT_SUFFIX: dict[str, str] = {'millimeters': '_mm', 'inches': '_in'}
...
#:
#: `Material Number` is the guid seed — see `identity.record_guid` — so this is
#: the one convention whose erosion is not cosmetic.
IDENTITY_COLUMNS: tuple[str, ...] = ('Material Number', 'ISO Catalog Number')
...
#: happened.
IDENTITY_DEVIATIONS: dict[str, tuple[str, ...]] = {
    'destinytool': ('itemNumber',),
}


def dimensional_column(label: str, unit: str) -> str:
    """A vendor's bare column label, suffixed for `unit`.

    Raises rather than defaulting on an unknown unit system: the two callers
    that could pass one are a family config and a CLI flag, and a typo that
    silently picked millimetres would produce a clean conversion with the
    wrong numbers in it.
    """
    if unit not in UNIT_SUFFIX:
        raise SystemExit(
            f'{label}: unknown unit system {unit!r} '
            f'(known: {sorted(UNIT_SUFFIX)})')
    return label + UNIT_SUFFIX[unit]


def identity_columns(brand: str) -> tuple[str, ...]:
    """The columns that identify a part in `brand`'s CSV.

    {@link IDENTITY_COLUMNS} unless the brand is listed as a deviation, which
    is a lookup rather than a guess: a caller cannot resolve this by inspecting
    a header, because a header that is missing `Material Number` is
    indistinguishable from a scrape that lost it.
    """
    return IDENTITY_DEVIATIONS.get(brand, IDENTITY_COLUMNS)


def check_identity_columns(brand: str, header: set[str]) -> None:
    """Every identity column `brand` claims is really in the CSV header.

    The failure this prevents: a re-scrape whose part-number column moved or
    was renamed produces a CSV that still parses, still has the right number of
    rows, and mints every guid off an empty string.
    """
    missing = sorted(set(identity_columns(brand)) - header)
    if missing:
        raise SystemExit(
            f'{brand}: identity column(s) absent from the CSV: '
            f'{", ".join(missing)}')
