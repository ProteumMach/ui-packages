...
why there is no `family_code` either. The scrape target is a set of index
...
HOLDER_FAMILIES: dict[str, dict] = {
    ...
    # `vendors/regofix/scrape.py`'s `SCRAPED_TAPERS` — because nothing on the
    # vendor's site says what OM designates.
    'regofix_bt30_pg_holders.csv': {
        'catalog_name': 'REGO-FIX powRgrip BT30 Toolholders',
        'rows': 21,
        'brand': 'regofix',
        'facts': {
            'taper': Fact('BT30', 'vendor-stated',
                          cite="every row's DIN 4000 J1 property is "
                               "DINISO7388-2 — the same 7/24 cone whether or "
                               "not the flange face seats"),
            'clamping': Fact('collet', 'vendor-stated',
                             cite='the holder publishes a CST collet series, so it '
                                  'grips'
                                  'through a collet'),
            'style': Fact('pg-collet-chuck', 'vendor-stated',
...
COLLET_FAMILIES: dict[str, dict] = {
    ...
    'regofix_pg_collets_standard.csv': {
        'catalog_name': 'REGO-FIX powRgrip PG Standard Collets',
        'rows': 71,
        'brand': 'regofix',
        'facts': {
            'style': Fact('pg-standard', 'vendor-stated',
                          cite="the ProductFinder index groups these under "
                               "product_group_name 'Standard'"),
        },
    },
    'regofix_pg_collets_coolant_flush.csv': {
        'catalog_name': 'REGO-FIX powRgrip PG Coolant Flush Collets',
        'rows': 51,
        'brand': 'regofix',
        'facts': {
            'style': Fact('pg-coolant-flush', 'vendor-stated',
                          cite="the ProductFinder index groups these under "
                               "product_group_name 'Coolant flush'"),
        },
    },
    'regofix_pg_collets_short.csv': {
        'catalog_name': 'REGO-FIX powRgrip PG Short Collets',
        'rows': 34,
        'brand': 'regofix',
        'facts': {
            'style': Fact('pg-short', 'vendor-stated',
                          cite="the ProductFinder index groups these under "
                               "product_group_name 'Short'"),
        },
    },
    'regofix_pg_collets_cool_bore.csv': {
        'catalog_name': 'REGO-FIX powRgrip PG Cool Bore Collets',
        'rows': 30,
        'brand': 'regofix',
        'facts': {
            'style': Fact('pg-cool-bore', 'vendor-stated',
                          cite="the ProductFinder index groups these under "
                               "product_group_name 'Cool bore'"),
        },
    },
    'regofix_pgst_collets.csv': {
        'catalog_name': 'REGO-FIX powRgrip PGST Short Tail Collets',
        'rows': 28,
        'brand': 'regofix',
        'facts': {
            'style': Fact('pgst-short-tail', 'vendor-stated',
                          cite="the ProductFinder index groups these under "
                               "product_group_name 'PGST'"),
        },
    },
    'regofix_pg_collets_tap.csv': {
        'catalog_name': 'REGO-FIX powRgrip PG Tapping Collets',
        'rows': 26,
        'brand': 'regofix',
        'facts': {
            'style': Fact('pg-tap', 'vendor-stated',
                          cite="the ProductFinder index groups these under "
                               "product_group_name 'Tapping collet TAP'"),
        },
    },
    'regofix_pg_collets_long.csv': {
        'catalog_name': 'REGO-FIX powRgrip PG Long Collets',
        'rows': 21,
        'brand': 'regofix',
        'facts': {
            'style': Fact('pg-long', 'vendor-stated',
                          cite="the ProductFinder index groups these under "
                               "product_group_name 'Long'"),
        },
    },
    'regofix_pg_collets_microbore.csv': {
        'catalog_name': 'REGO-FIX powRgrip PG Microbore Collets',
        'rows': 16,
        'brand': 'regofix',
        'facts': {
            'style': Fact('pg-microbore', 'vendor-stated',
                          cite="the ProductFinder index groups these under "
                               "product_group_name 'Microbore'"),
        },
    },
    'regofix_pg_collets_turning.csv': {
        'catalog_name': 'REGO-FIX powRgrip PG Turning Collets',
        'rows': 16,
        'brand': 'regofix',
        'facts': {
            'style': Fact('pg-turning', 'vendor-stated',
                          cite="the ProductFinder index groups these under "
                               "product_group_name 'PG-T'"),
        },
    },
    'regofix_pg_collets_mql.csv': {
        'catalog_name': 'REGO-FIX powRgrip PG MQL Collets',
        'rows': 11,
        'brand': 'regofix',
        'facts': {
            'style': Fact('pg-mql', 'vendor-stated',
                          cite="the ProductFinder index groups these under "
                               "product_group_name 'PG-MQL'"),
        },
    },
    'regofix_pg_collets_securgrip.csv': {
        'catalog_name': 'REGO-FIX powRgrip PG secuRgrip Collets',
        'rows': 10,
        'brand': 'regofix',
        'facts': {
            'style': Fact('pg-securgrip', 'vendor-stated',
                          cite="the ProductFinder index groups these under "
                               "product_group_name 'secuRgrip'"),
        },
    },
    'regofix_pg_collets_sealed_cap.csv': {
        'catalog_name': 'REGO-FIX powRgrip PG Sealed Cap Collets',
        'rows': 7,
        'brand': 'regofix',
        'facts': {
            'style': Fact('pg-sealed-cap', 'vendor-stated',
                          cite="the ProductFinder index groups these under "
                               "product_group_name 'PG-SC'"),
        },
    },
}
