...
`family_code` is the vendor's numeric family code, lifted out of the family
...
**Holder families carry no `family_code`**, and that is a gap rather than a
...
FAMILIES: dict[str, dict] = {
    ...
    #
    # `non_ferrous` is required too, and deliberately has no default: it drops
    ...
    'godrill_3xd_metric.csv': {
        ...
        'rows': 259,
        'family_code': '100003658',
        'kind': 'drill',
        'columns': {'DC': 'D1', 'SFDM': 'D', 'OAL': 'L', 'LCF': 'L3'},
        'facts': {
            'unit': Fact('millimeters', 'vendor-stated',
                         cite='the vendor titles the family "…3xD straight '
                              'shank metric"; both unit columns are published, so this '
                              'decides which is displayed'),
            'flutes': Fact(2, 'assumed',
                           note='no flute-count column; every GOdrill in this '
                                'family is a two-flute twist drill',
                           checked='2026-07-24', by='JG'),
            'point_angle': Fact(140, 'assumed',
                                note='no point angle is published anywhere on '
                                     'this family; 140° is the common GOdrill '
                                     'grind. Contrast the KenDrill TXD '
                                     'families, where L5 lets it be derived',
                                checked='2026-07-24', by='JG'),
            'coolant_through': Fact(False,
                                    'assumed',
                                    note='the table publishes no coolant column '
                                         'and the '
                                         'family page shows none',
                                    checked='2026-08-05',
                                    by='JG'),
            'non_ferrous': Fact(False, 'vendor-stated',
                                cite='the workpiece-material facet returns '
                                     'P M K N S H — ferrous groups included'),
            'bmc': Fact('carbide',
                        'vendor-stated',
                        cite='the Grade column names a Kennametal carbide grade '
                             '(KC7325, KCU20)'),
        },
    },
    'kenna_universal_3xd_metric.csv': {
        ...
        'rows': 177,
        'family_code': '100004307',
        'kind': 'drill',
        'columns': {'DC': 'D1', 'SFDM': 'D', 'OAL': 'L', 'LCF': 'L3'},
        'facts': {
            'unit': Fact('millimeters', 'vendor-stated',
                         cite='the vendor titles the family "…3xD IC straight '
                              'shank metric"; both unit columns are published'),
            'flutes': Fact(2, 'assumed',
                           note='no flute-count column; a two-flute twist drill',
                           checked='2026-07-24', by='JG'),
            'point_angle': Fact(140, 'assumed',
                                note='no point angle published on this family; '
                                     'the same grind assumed as GOdrill',
                                checked='2026-07-24', by='JG'),
            'coolant_through': Fact(False,
                                    'assumed',
                                    note='the table publishes no coolant column '
            ...
            'non_ferrous': Fact(False, 'vendor-stated',
                                cite='the workpiece-material facet returns '
                                     'P M K N S (no H) — ferrous groups included'),
            'bmc': Fact('carbide',
                        'vendor-stated',
                        cite='the Grade column names a Kennametal carbide grade '
                             '(KC7325, KCU20)'),
    ...
    #
    # **`point_angle` is 142, not the 140 the two families above assume**, and
    ...
    #
    # **`coolant_through` is True — the first True in this table.** Unlike the
    # 140° above it is not an assumption: "Internal Coolant" is in the vendor's
    # own family title on both codes.
    #
    # **`bmc` is 'diamond' and `non_ferrous` is True, and they are the same
    ...
    'kendrill_txd_5xd_inch.csv': {
        ...
        'rows': 20,
        'family_code': '100165153',
        'kind': 'drill',
        'columns': {'DC': 'D1', 'SFDM': 'D', 'OAL': 'L', 'LCF': 'L3'},
        'facts': {
            'unit': Fact('inches', 'vendor-stated',
                         cite='catalog numbers are K467A… encoding the size in '
                              'ten-thousandths of an inch (K467A02344SP is '
                              '15/64), and the inch column carries the exact '
                              'value while the metric one is rounded'),
            'flutes': Fact(2, 'assumed',
                           note='no flute-count column; the vendor\'s "Straight '
                                'Fluted" names the helix, not the count',
                           checked='2026-08-06', by='JG'),
            'point_angle': Fact(142, 'derived',
                                note='no point angle is published on either '
                                     'family, either variant table or a product '
                                     'page. L5 (point length) is, and for a '
                                     'conical point L5 = D1 / (2·tan(SIG/2)). '
                                     'Least squares over all 49 rows gives '
                                     '142.46° inch and 142.55° metric; at 142° '
                                     'the worst L5 residual is 0.05 mm — the '
                                     'vendor\'s own rounding — against 0.24 mm '
                                     'at 140°, which is ruled out'),
            'coolant_through': Fact(True, 'vendor-stated',
                                    cite='"Internal Coolant" is in the vendor\'s '
                                         'own family title'),
            'non_ferrous': Fact(True, 'vendor-stated',
                                cite='grade KD1415 is "PCD-tip brazed to carbide '
                                     'for … aluminum …, non-ferrous heavy metals, and '
                                     'plastics"; breadcrumb "PCD Tooling / PCD '
                                     'Drills • Aluminum Machining"; the facet '
                                     'returns N1-N4 and nothing else'),
            'bmc': Fact('diamond', 'vendor-stated',
                        cite='grade KD1415 is PCD — the cutting material, not '
                             'the carbide body it is brazed to'),
        },
    },
    'kendrill_txd_5xd_metric.csv': {
        'id': 'kendrill-txd-5xd-metric',
        'rows': 29,
        'family_code': '100164033',
        'kind': 'drill',
        'columns': {'DC': 'D1', 'SFDM': 'D', 'OAL': 'L', 'LCF': 'L3'},
        'facts': {
            'unit': Fact('millimeters', 'vendor-stated',
                         cite='catalog numbers are B467A… encoding the size in '
                              'hundredths of a millimetre (B467A06000SP is '
                              '6.00 mm), and the metric column carries the '
                              'exact value while the inch one is rounded'),
            'flutes': Fact(2, 'assumed',
                           note='no flute-count column; the vendor\'s "Straight '
                                'Fluted" names the helix, not the count',
                           checked='2026-08-06', by='JG'),
    ...
    'khsst_spiral_point_plug_inch.csv': {
        ...
        'rows': 95,
        'family_code': '100004132',
        'kind': 'tap',
        'columns': {'SFDM': 'D', 'OAL': 'L', 'LCF': 'L3',
                    'TP': 'Thread Pitch'},
        'facts': {
            'bmc': Fact('hss', 'assumed',
                        note='no substrate column; carried over from the '
                             'pre-2026-08-08 config, which recorded no source '
                             'for it. The KHSST line name implies high-speed '
                             'steel and the Coating column carries the surface '
                             'treatment instead of a carbide grade, but neither '
                             'is a vendor statement of substrate',
    ...
    'gomill_pro_radiused_4fl_necked_metric.csv': {
        ...
        'rows': 60,
        'family_code': '109353075',
        'kind': 'endmill',
        'columns': {'DC': 'D1', 'SFDM': 'D', 'OAL': 'L', 'LCF': 'AP1MAX',
                    'RE': 'Re', 'shoulder-length': 'L3',
                    'shoulder-diameter': 'D3'},
        'facts': {
            'unit': Fact('millimeters', 'vendor-stated',
                         cite='the vendor titles the family "…necked plain shank '
                              'metric"; both unit columns are published, so'
                              'this decides which is displayed'),
            'coolant_through': Fact(False,
                                    'assumed',
                                    note='the table publishes no coolant column '
                                         'and the '
    ...
    'gomill_pro_square_4fl_plain_inch.csv': {
        ...
        'rows': 93,
        'family_code': '109426909',
        'kind': 'endmill',
        'columns': {'DC': 'D1', 'SFDM': 'D', 'OAL': 'L', 'LCF': 'AP1MAX',
                    'shoulder-length': 'L3'},
        'facts': {
            'unit': Fact('inches', 'vendor-stated',
                         cite='the vendor titles the family "…plain shank inch"; both '
                              'unit columns are published, so'
                              'this decides which is displayed'),
            'coolant_through': Fact(False,
                                    'assumed',
                                    note='the table publishes no coolant column '
    ...
    'varimill_chip_splitter_radiused_5fl_3xd_plain_metric.csv': {
        ...
        'facts': {
            'unit': Fact('millimeters', 'vendor-stated',
                         cite='the vendor titles the family "…3xD plain shank metric"; '
                              'both unit columns are published, so'
                              'this decides which is displayed'),
            'coolant_through': Fact(False,
                                    'assumed',
                                    note='the table publishes no coolant column '
    ...
    'varimill_chip_splitter_570t_radiused_5fl_cyl_inch.csv': {
        ...
        'facts': {
            'unit': Fact('inches', 'vendor-stated',
                         cite='the vendor titles the family "…cylindrical shank inch"; '
                              'both unit columns are published, so'
                              'this decides which is displayed'),
            'coolant_through': Fact(False,
                                    'assumed',
    ...
    # and L3 is published. `coolant_through` False like every other family —
    ...
    'maximet_square_3fl_wiper_necked_plain_inch.csv': {
        ...
        'rows': 14,
        'family_code': '101273936',
        'kind': 'endmill',
        'columns': {'DC': 'D1', 'SFDM': 'D', 'OAL': 'L', 'LCF': 'AP1MAX',
                    'shoulder-length': 'L3', 'shoulder-diameter': 'D3'},
        'facts': {
            'unit': Fact('inches', 'vendor-stated',
                         cite='the vendor titles the family "…necked plain shank '
                              'inch"; '
...
HOLDER_FAMILIES: dict[str, dict] = {
    'bt30_er_collet_adapters_metric.csv': {
        'catalog_name': 'Kennametal BT30 ER Collet Adapters Metric',
        'rows': 12,
        'facts': {
            'taper': Fact('BT30', 'vendor-stated',
                          cite='the family page states the shank as JIS B 6339 / MAS '
                               '403'
                               'size 30'),
            'contact': Fact('taper', 'vendor-stated',
                            cite='the family page says "Shank - SK BT JIS B 6339" with '
                                 'no face-contact claim'),
            'clamping': Fact('collet', 'vendor-stated',
                             cite='the holder publishes a CST collet series, so it '
                                  'grips'
                                  'through a collet'),
            'style': Fact('er-collet-chuck', 'vendor-stated',
                          cite='breadcrumb ".../ER Collet Chucks/ER(tm) Collet Adapter '
                               '-'
                               'BT30"'),
            'unit': Fact('millimeters', 'vendor-stated',
                         cite='the family is titled and catalogued in this '
                              'system; both '
                              'unit columns are usually published, so this decides '
                              'which'
                              'is displayed'),
        },
    },
    'bt30_hydraulic_chucks_form_ad_metric.csv': {
        'catalog_name': 'Kennametal BT30 Hydraulic Chucks Form AD Metric',
        'rows': 8,
        'facts': {
            'taper': Fact('BT30', 'vendor-stated',
                          cite='the family page states the shank as JIS B 6339 / MAS '
            ...
            'contact': Fact('taper', 'vendor-stated',
                            cite='the family page says "Shank - SK BT JIS B 6339" with '
                                 'no face-contact claim'),
            'clamping': Fact('bore', 'vendor-stated',
                             cite='the holder publishes a D1 bore and no collet '
                                  'series, so it grips the shank directly'),
            'style': Fact('hydraulic-chuck', 'vendor-stated',
    ...
    # thing that decides which column `holder_record` displays. Both unit
    ...
    'bt30_hydraulic_chucks_form_ad_inch.csv': {
        'catalog_name': 'Kennametal BT30 Hydraulic Chucks Form AD Inch',
        'rows': 3,
        'facts': {
            'taper': Fact('BT30', 'vendor-stated',
                          cite='the family page states the shank as JIS B 6339 / MAS '
    ...
    # `holder_record`'s rule about published columns nothing displays. Promote
    ...
    'btkv30_er_collet_chucks_metric.csv': {
        'catalog_name': 'Kennametal BTKV30 ER Collet Chucks Metric',
        'rows': 5,
        'facts': {
            'taper': Fact('BT30', 'vendor-stated',
                          cite='the family page states the shank as JIS B 6339 / MAS '
                               '403'
                               'size 30'),
            'contact': Fact('face', 'vendor-stated',
                            cite='the product page names the shank "SK BT Taper Face '
                                 'Contact"'),
            'clamping': Fact('collet', 'vendor-stated',
                             cite='the holder publishes a CST collet series, so it '
                                  'grips'
    ...
    'bt30_shrink_fit_fc_form_ad_metric.csv': {
        'catalog_name': 'Kennametal BT30 Shrink Fit FC Line Form AD Metric',
        'rows': 6,
        'facts': {
            'taper': Fact('BT30', 'vendor-stated',
                          cite='the family page states the shank as JIS B 6339 / MAS '
            ...
            'clamping': Fact('bore', 'vendor-stated',
                             cite='the holder publishes a D1 bore and no collet '
                                  'series, so it grips the shank directly'),
            'style': Fact('shrink-fit-fc', 'vendor-stated',
                          cite='breadcrumb ".../Shrink Fit Toolholders"; tagline '
                               '"Standard Heat Shrink Holders … Face Coolant … BT30 '
                               'Backend"'),
            'unit': Fact('millimeters', 'vendor-stated',
                         cite='the family is titled and catalogued in this '
                              'system; both '
    ...
    'bt30_shrink_fit_fc_form_ad_inch.csv': {
        'catalog_name': 'Kennametal BT30 Shrink Fit FC Line Form AD Inch',
        'rows': 6,
        'facts': {
            'taper': Fact('BT30', 'vendor-stated',
                          cite='the family page states the shank as JIS B 6339 / MAS '
    ...
    # (0.4375 / 11.113). `test_toolholding.py` pins both halves of that, so a
    ...
    'bt30_shrink_fit_hpv_form_ad_metric.csv': {
        'catalog_name': 'Kennametal BT30 Shrink Fit HPV GP Form AD Metric',
        'rows': 7,
        'facts': {
            'taper': Fact('BT30', 'vendor-stated',
                          cite='the family page states the shank as JIS B 6339 / MAS '
            ...
            'clamping': Fact('bore', 'vendor-stated',
                             cite='the holder publishes a D1 bore and no collet '
                                  'series, so it grips the shank directly'),
            'style': Fact('shrink-fit-gp', 'vendor-stated',
                          cite='same category; tagline "Shrink Fit Toolholders General '
                               'Purpose (GP)"'),
            'unit': Fact('millimeters', 'vendor-stated',
                         cite='the family is titled and catalogued in this '
                              'system; both '
    ...
    'bt30_shrink_fit_hpv_form_ad_inch.csv': {
        'catalog_name': 'Kennametal BT30 Shrink Fit HPV GP Form AD Inch',
        'rows': 6,
        'facts': {
            'taper': Fact('BT30', 'vendor-stated',
                          cite='the family page states the shank as JIS B 6339 / MAS '
...
COLLET_FAMILIES: dict[str, dict] = {
    'er_standard_collets_metric.csv': {
        'catalog_name': 'Kennametal ER Standard Collets Metric',
        'rows': 110,
        'facts': {
            'style': Fact('er-standard', 'vendor-stated',
    ...
    'er16_collets_coolant_through_inch.csv': {
        'catalog_name': 'Kennametal ER16 Collets Coolant-Through Inch',
        'rows': 10,
        'facts': {
            'style': Fact('er-sealed', 'vendor-stated',
                          cite='CCCX == CCCN == D1 on every row; Kennametal specs'
                               ' an H6 '
                               'shank, so it clamps one exact size'),
            'unit': Fact('inches', 'vendor-stated',
                         cite='the family is titled and catalogued in this '
                              'system; both '
                              'unit columns are usually published, so this decides '
